...
(use-package ivy
  ...
  ;;                         "~/src/emacs/swiper.git")) ; to hack the file
  :commands ivy-mode
  :init (ivy-mode)
  :config
  (setq
...
(custom-set-variables
 ...
 '(package-selected-packages
   (quote
    (company-prescient ivy-prescient try helm-company company-quickhelp projectile company-jedi jedi-direx yasnippet-snippets iedit rbs-mode elpy counsel ivy helm-dash lsp-pyright use-package lsp-ui lsp-jedi lsp-mode gnuplot magit multiple-cursors idle-highlight-mode color-identifiers-mode auto-complete dracula-theme)))
 '(save-place t)
 '(scroll-bar-mode nil)
...
(define-key yas-minor-mode-map (kbd "C-c k") 'yas-expand) ;; fixing a keybinding bug in elpy
;;----- iedit -----
(define-key global-map (kbd "C-c O") 'iedit-mode) ;; fixing another keybinding bug in iedit mode
;;----------
;; Load path
;;-----
...
;;----------
(setq compile-command (concat "pdflatex -shell-escape " buffer-file-truename)) ;; original: `(setq ... "make package install")'
;;-----
...
(global-set-key (kbd "C-;") 'next-buffer) ; former : undefined
(global-set-key (kbd "C-:") 'previous-buffer) ; former : undefined
...
(global-set-key (kbd "C-z") 'save-buffer) ; former : suspend-frame
(global-set-key (kbd "C-\"") 'kill-buffer) ; former : undefined
(global-set-key (kbd "C-Z") 'kill-buffer) ; former : undefined
